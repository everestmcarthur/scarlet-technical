-- Migration 021: SMS Automation + Email System + Feature Mega Pack
-- Adds: SMS conversations, ticket messages, email system, customer tags,
--        repair kanban/priority/templates, coupons, late fees, deposits,
--        knowledge base, canned responses, notification center, and more.
...
CREATE INDEX IF NOT EXISTS sms_phone_idx ON sms_messages (phone, created_at DESC);
CREATE INDEX IF NOT EXISTS sms_customer_idx ON sms_messages (customer_id);
CREATE INDEX IF NOT EXISTS sms_unread_idx ON sms_messages (phone, direction, read) WHERE read = false;
...
-- Add sms_opt_in to customers if not exists
ALTER TABLE customers ADD COLUMN IF NOT EXISTS sms_opt_in BOOLEAN DEFAULT true;

-- Add sms_phone and source to support_tickets
ALTER TABLE support_tickets ADD COLUMN IF NOT EXISTS sms_phone VARCHAR(20);
ALTER TABLE support_tickets ADD COLUMN IF NOT EXISTS source VARCHAR(20) DEFAULT 'portal';

-- Ticket conversation messages (multi-channel)
...
CREATE INDEX IF NOT EXISTS ticket_msg_ticket_idx ON ticket_messages (ticket_id, created_at);
...
ALTER TABLE customers ADD COLUMN IF NOT EXISTS comm_pref VARCHAR(20) DEFAULT 'both' CHECK (comm_pref IN ('email', 'sms', 'both', 'none'));
ALTER TABLE customers ADD COLUMN IF NOT EXISTS emergency_contact VARCHAR(255);
...
-- Add fields to repairs
ALTER TABLE repairs ADD COLUMN IF NOT EXISTS priority VARCHAR(20) DEFAULT 'normal' CHECK (priority IN ('normal', 'rush', 'emergency'));
ALTER TABLE repairs ADD COLUMN IF NOT EXISTS sla_deadline TIMESTAMPTZ;
ALTER TABLE repairs ADD COLUMN IF NOT EXISTS assigned_tech INTEGER REFERENCES admin_users(id) ON DELETE SET NULL;
ALTER TABLE repairs ADD COLUMN IF NOT EXISTS template_id INTEGER REFERENCES repair_templates(id) ON DELETE SET NULL;
ALTER TABLE repairs ADD COLUMN IF NOT EXISTS kanban_position INTEGER DEFAULT 0;
ALTER TABLE repairs ADD COLUMN IF NOT EXISTS diagnosis_report TEXT;
ALTER TABLE repairs ADD COLUMN IF NOT EXISTS internal_notes TEXT;
ALTER TABLE repairs ADD COLUMN IF NOT EXISTS bench_fee NUMERIC(10,2) DEFAULT 0;
ALTER TABLE repairs ADD COLUMN IF NOT EXISTS related_repair_id INTEGER REFERENCES repairs(id) ON DELETE SET NULL;
ALTER TABLE repairs ADD COLUMN IF NOT EXISTS is_warranty_claim BOOLEAN DEFAULT false;
ALTER TABLE repairs ADD COLUMN IF NOT EXISTS parts_consumed JSONB DEFAULT '[]';

-- Before/after photos
ALTER TABLE repairs ADD COLUMN IF NOT EXISTS before_photos JSONB DEFAULT '[]';
ALTER TABLE repairs ADD COLUMN IF NOT EXISTS after_photos JSONB DEFAULT '[]';
...
ALTER TABLE payments ADD COLUMN IF NOT EXISTS payment_method VARCHAR(20) DEFAULT 'card' CHECK (payment_method IN ('card', 'cash', 'check', 'zelle', 'venmo', 'paypal', 'credit', 'other'));
...
CREATE INDEX IF NOT EXISTS kb_slug_idx ON kb_articles (slug);
CREATE INDEX IF NOT EXISTS kb_category_idx ON kb_articles (category);

-- Ticket SLA and priority
ALTER TABLE support_tickets ADD COLUMN IF NOT EXISTS priority VARCHAR(20) DEFAULT 'normal' CHECK (priority IN ('low', 'normal', 'high', 'urgent'));
ALTER TABLE support_tickets ADD COLUMN IF NOT EXISTS sla_deadline TIMESTAMPTZ;
ALTER TABLE support_tickets ADD COLUMN IF NOT EXISTS assigned_to INTEGER REFERENCES admin_users(id) ON DELETE SET NULL;
ALTER TABLE support_tickets ADD COLUMN IF NOT EXISTS category VARCHAR(100);
ALTER TABLE support_tickets ADD COLUMN IF NOT EXISTS satisfaction_rating INTEGER CHECK (satisfaction_rating BETWEEN 1 AND 5);
...
CREATE INDEX IF NOT EXISTS notifications_admin_idx ON notifications (admin_id, read, created_at DESC);
CREATE INDEX IF NOT EXISTS notifications_customer_idx ON notifications (customer_id, read, created_at DESC);
...
-- Add supplier and reorder fields to parts
ALTER TABLE parts ADD COLUMN IF NOT EXISTS supplier_id INTEGER REFERENCES suppliers(id) ON DELETE SET NULL;
ALTER TABLE parts ADD COLUMN IF NOT EXISTS reorder_point INTEGER DEFAULT 5;
ALTER TABLE parts ADD COLUMN IF NOT EXISTS reorder_quantity INTEGER DEFAULT 10;
ALTER TABLE parts ADD COLUMN IF NOT EXISTS location VARCHAR(100);
ALTER TABLE parts ADD COLUMN IF NOT EXISTS barcode VARCHAR(100);
ALTER TABLE parts ADD COLUMN IF NOT EXISTS warranty_months INTEGER DEFAULT 0;
